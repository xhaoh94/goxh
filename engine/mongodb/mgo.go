...
func ApplyDefaultUrl() {
	url := "mongodb://"
	if config.AppCfg.MongoDb.User != "" && config.AppCfg.MongoDb.Password != "" {
		url += config.AppCfg.MongoDb.User + "@" + config.AppCfg.MongoDb.Password + ":"
	}
	url += config.AppCfg.MongoDb.Url
	ApplyURI(url)
}
...
func GetDefaultDatabase() *mongo.Database {
	return client.Database(config.AppCfg.MongoDb.Database)
}
